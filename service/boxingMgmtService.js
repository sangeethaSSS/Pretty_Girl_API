    ...
    if (req.jwtToken) {
      ...
      if (decoded) { 
        ...
        if(process == 'requiredstock'){
         ...
         tbl_fg_items WHERE size_id = t1.size_id ),0) + COALESCE((SELECT sum(goods_return_set) from tbl_goods_return WHERE status_flag = 1 and size_id = a.size_id ),0) as fg_set,COALESCE((SELECT sum(no_of_pieces) from tbl_fg_items WHERE size_id = t1.size_id ),0) + COALESCE((SELECT sum(goods_return_pieces) from tbl_goods_return WHERE status_flag = 1 and size_id = a.size_id ),0) as fg_pieces from  (SELECT d.design_id,b.size_id,d.item_code,sum(b.qty) as qty,sum(qty*COALESCE(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a INNER JOIN tbl_order_taking_items as b on a.order_no =b.order_no INNER JOIN tbl_item_sizes as c ON c.size_id = b.size_id INNER JOIN tbl_item_management as d on d.trans_no=c.trans_no INNER JOIN tbl_customer as e ON e.customer_code = a.customer_code LEFT JOIN tbl_color as  f on f.color_id =c.color_id WHERE d.item_code = ${item_code} AND ${designid_val}  AND ${getsettype} AND a.status_code = 1 GROUP BY d.design_id, b.size_id,d.item_code
                    union all
         ...
         status_flag = 1 AND size_id = a.size_id ),0),0) as pending_set from tbl_fg_items as a GROUP BY size_id) as dev WHERE current_set > pending_set) GROUP BY item_code,design_id,t1.size_id) as t2  WHERE COALESCE((qty-fg_set),0) > 0  ) as dev GROUP BY item_code,design_id ORDER BY design_id`)
          ...
          sum(not_ready_set) as not_ready_set,sum(not_ready_pieces) as not_ready_pieces
           ...
           ,COALESCE((order_pieces- fg_pieces ),0) as not_ready_pieces from (SELECT item_code,design_id,size_id, sum(qty) as qty,sum(order_pieces) as order_pieces,sum(dispatch_set) as 
            dispatch_set, sum(dispatch_pieces) as dispatch_pieces,COALESCE((SELECT sum(no_of_set) from 
         tbl_fg_items WHERE size_id = t1.size_id ),0) + COALESCE((SELECT sum(goods_return_set) from tbl_goods_return WHERE status_flag = 1 and size_id = a.size_id ),0) as fg_set,COALESCE((SELECT sum(no_of_pieces) + COALESCE((SELECT sum(goods_return_pieces) from tbl_goods_return WHERE status_flag = 1 and size_id = a.size_id ),0) from tbl_fg_items WHERE size_id = t1.size_id ),0) as fg_pieces from  (SELECT d.design_id,b.size_id,d.item_code,sum(b.qty) as qty,sum(qty*COALESCE(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a INNER JOIN tbl_order_taking_items as b on a.order_no =b.order_no INNER JOIN tbl_item_sizes as c ON c.size_id = b.size_id INNER JOIN tbl_item_management as d on d.trans_no=c.trans_no INNER JOIN tbl_customer as e ON e.customer_code = a.customer_code LEFT JOIN tbl_color as  f on f.color_id =c.color_id WHERE d.item_code = ${item_code} AND ${designid_val}  AND ${getsettype} AND a.status_code = 1 GROUP BY d.design_id, b.size_id,d.item_code
                    union all
          SELECT d.design_id,b.size_id,d.item_code,0 as qty, 0 as order_pieces,sum(dispatch_set) as dispatch_set,sum(dispatch_pieces) as dispatch_pieces from tbl_dispatch_details as b INNER JOIN     
           tbl_order_taking as a on a.order_no = b.order_no INNER JOIN tbl_item_sizes as c ON c.size_id = 
           b.size_id INNER JOIN tbl_item_management as d on d.trans_no=c.trans_no INNER JOIN tbl_customer 
          ...
          let Required_stock_array = Required_stock && Required_stock.rows ? Required_stock.rows : []; 
            ...
            if (Required_stock_array.length > 0) {
              for (let i = 0; i < Required_stock_array.length; i++) {
                 const item_Result = await client.query(`SELECT design_id,item_code,size_id,qr_code,COALESCE((qty-fg_set),0) as not_ready_set,COALESCE((order_pieces-fg_pieces),0) as not_ready_pieces from (SELECT design_id,item_code,size_id,qr_code, sum(qty) as qty,sum(order_pieces) as order_pieces,sum(dispatch_set) as dispatch_set,sum(dispatch_pieces) as dispatch_pieces,sum(fg_set) as  fg_set,sum(fg_pieces) as fg_pieces from (SELECT design_id,item_code,size_id,qr_code,sum(qty) as qty,sum(order_pieces) as order_pieces,sum(dispatch_set) as dispatch_set, sum(dispatch_pieces) as dispatch_pieces,COALESCE((SELECT sum(no_of_set) from tbl_fg_items WHERE size_id = t1.size_id ),0) + COALESCE((SELECT sum(goods_return_set) from tbl_goods_return WHERE status_flag = 1 and size_id = a.size_id ),0) as fg_set,COALESCE((SELECT sum(no_of_pieces) from tbl_fg_items WHERE size_id = t1.size_id ),0) + COALESCE((SELECT sum(goods_return_pieces) from tbl_goods_return WHERE status_flag = 1 and size_id = a.size_id ),0) as fg_pieces from (SELECT d.design_id,b.size_id,d.item_code,c.qr_code,sum(b.qty) as qty,sum(qty*COALESCE(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a INNER JOIN tbl_order_taking_items as b on a.order_no =b.order_no INNER JOIN tbl_item_sizes as c ON c.size_id = b.size_id INNER JOIN tbl_item_management as d on d.trans_no=c.trans_no LEFT JOIN tbl_color as f on f.color_id =c.color_id INNER JOIN tbl_customer as e ON e.customer_code = a.customer_code WHERE ${designid_val}  AND ${getsettype} AND a.status_code = 1 AND d.design_id = $1 GROUP BY d.design_id,b.size_id,
