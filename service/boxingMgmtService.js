    ...
    if (req.jwtToken) {
      ...
      if (decoded) { 
        ...
        if(process == 'print') { 
          ...
          const NotReadyForDispatch = await client.query(`SELECT item_name,f.item_id,not_ready_set as not_ready_set,not_ready_pieces as not_ready_pieces from (SELECT item_code, sum(not_ready_set) as not_ready_set,sum(not_ready_pieces) as not_ready_pieces from (SELECT item_code,size_id, coalesce((qty-fg_set),0) as not_ready_set,coalesce((order_pieces- fg_pieces ),0) as not_ready_pieces from 
          ...
          tbl_fg_items where size_id = t1.size_id ),0) as fg_set,coalesce((SELECT sum(no_of_pieces) 
          from tbl_fg_items where size_id = t1.size_id ),0) as fg_pieces from (SELECT b.size_id,d.item_code,sum(b.qty) as qty,sum(qty*coalesce(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no =b.order_no inner join tbl_item_sizes as c  ON c.size_id = b.size_id inner join tbl_item_management as d on d.trans_no=c.trans_no inner join tbl_customer as e ON e.customer_code = a.customer_code left join tbl_color as f on f.color_id =c.color_id where ${designid_val}  and ${getsettype} and a.status_code = 1 group by b.size_id,d.item_code
                     union all
          ...
          (SELECT size_id, coalesce(sum(coalesce(no_of_set,0))- coalesce((SELECT sum(dispatch_set) 
          from tbl_dispatch_details where status_flag = 1 and size_id = a.size_id ),0),0) as       
           current_set,coalesce(coalesce((SELECT sum(qty) from tbl_order_taking_items where status_code = 1 and size_id = a.size_id ),0) - coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where 
          status_flag = 1 and size_id = a.size_id ),0),0) as pending_set from tbl_fg_items as a group by size_id) as dev where current_set > pending_set) group by item_code,t1.size_id) as t2  where 
           coalesce((qty-fg_set),0) > 0) as dev group by item_code) as d1 right join tbl_def_item  as f on d1.item_code = f.item_id `)
    ...
    if (req.jwtToken) {
      ...
      if (decoded) { 
        ...
        if(process == 'requiredstock'){
         ...
         tbl_fg_items where size_id = t1.size_id ),0) as fg_set,coalesce((SELECT sum(no_of_pieces) 
         from tbl_fg_items where size_id = t1.size_id ),0) as fg_pieces from  (SELECT d.design_id,b.size_id,d.item_code,sum(b.qty) as qty,sum(qty*coalesce(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no =b.order_no inner join tbl_item_sizes as c ON c.size_id = b.size_id inner join tbl_item_management as d on d.trans_no=c.trans_no inner join tbl_customer as e ON e.customer_code = a.customer_code left join tbl_color as  f on f.color_id =c.color_id where d.item_code = ${item_code} and ${designid_val}  and ${getsettype} and a.status_code = 1 group by d.design_id, b.size_id,d.item_code
                    union all
          SELECT d.design_id,b.size_id,d.item_code,0 as qty, 0 as order_pieces,sum(dispatch_set) as dispatch_set,sum(dispatch_pieces) as dispatch_pieces from tbl_dispatch_details as b inner join     
           ...
           b.size_id inner join tbl_item_management as d on d.trans_no=c.trans_no inner join tbl_customer 
           as e ON e.customer_code = a.customer_code left join tbl_color as f on f.color_id =c.color_id 
           where status_flag = 1 and d.item_code = ${item_code} and ${designid_val}  and ${getsettype} and a.status_code = 1 group by d.design_id,b.size_id,d.item_code)as t1 where size_id not in (SELECT size_id from (SELECT size_id, coalesce(sum(coalesce(no_of_set,0))- coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where status_flag = 1 and size_id = a.size_id ),0),0) as    current_set,coalesce(coalesce((SELECT sum(qty) from tbl_order_taking_items where status_code = 1 and size_id = a.size_id ),0) - coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where 
         ...
         from tbl_fg_items where size_id = t1.size_id ),0) as fg_pieces from  (SELECT d.design_id,b.size_id,d.item_code,sum(b.qty) as qty,sum(qty*coalesce(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no =b.order_no inner join tbl_item_sizes as c ON c.size_id = b.size_id inner join tbl_item_management as d on d.trans_no=c.trans_no inner join tbl_customer as e ON e.customer_code = a.customer_code left join tbl_color as  f on f.color_id =c.color_id where d.item_code = ${item_code} and ${designid_val}  and ${getsettype} and a.status_code = 1 group by d.design_id, b.size_id,d.item_code
          ...
          SELECT d.design_id,b.size_id,d.item_code,0 as qty, 0 as order_pieces,sum(dispatch_set) as dispatch_set,sum(dispatch_pieces) as dispatch_pieces from tbl_dispatch_details as b inner join     
           ...
           as e ON e.customer_code = a.customer_code left join tbl_color as f on f.color_id =c.color_id 
           where status_flag = 1 and d.item_code = ${item_code} and ${designid_val}  and ${getsettype} and a.status_code = 1 group by d.design_id,b.size_id,d.item_code)as t1 where size_id not in (SELECT size_id from (SELECT size_id, coalesce(sum(coalesce(no_of_set,0))- coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where status_flag = 1 and size_id = a.size_id ),0),0) as    current_set,coalesce(coalesce((SELECT sum(qty) from tbl_order_taking_items where status_code = 1 and size_id = a.size_id ),0) - coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where 
         status_flag = 1 and size_id = a.size_id ),0),0) as pending_set from tbl_fg_items as a group by size_id) as dev where current_set > pending_set) group by item_code,design_id,t1.size_id) as t2  where coalesce((qty-fg_set),0) > 0  ) as dev group by item_code,design_id ${get_limit}`)
          let Required_stock_array = Required_stock && Required_stock.rows ? Required_stock.rows : []; 
            ...
            if (Required_stock_array.length > 0) {
              for (let i = 0; i < Required_stock_array.length; i++) {
                 const item_Result = await client.query(`SELECT design_id,item_code,size_id,qr_code,coalesce((qty-fg_set),0) as not_ready_set,coalesce((order_pieces-fg_pieces),0) as not_ready_pieces from (SELECT design_id,item_code,size_id,qr_code, sum(qty) as qty,sum(order_pieces) as order_pieces,sum(dispatch_set) as dispatch_set,sum(dispatch_pieces) as dispatch_pieces,sum(fg_set) as  fg_set,sum(fg_pieces) as fg_pieces from (SELECT design_id,item_code,size_id,qr_code,sum(qty) as qty,sum(order_pieces) as order_pieces,sum(dispatch_set) as dispatch_set, sum(dispatch_pieces) as dispatch_pieces,coalesce((SELECT sum(no_of_set) from tbl_fg_items where size_id = t1.size_id ),0) as fg_set,coalesce((SELECT sum(no_of_pieces) from tbl_fg_items where size_id = t1.size_id ),0) as fg_pieces from (SELECT d.design_id,b.size_id,d.item_code,c.qr_code,sum(b.qty) as qty,sum(qty*coalesce(c.total_set,'0')::Integer) as order_pieces,0 as dispatch_set, 0 as dispatch_pieces FROM tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no =b.order_no inner join tbl_item_sizes as c ON c.size_id = b.size_id inner join tbl_item_management as d on d.trans_no=c.trans_no left join tbl_color as f on f.color_id =c.color_id inner join tbl_customer as e ON e.customer_code = a.customer_code where ${designid_val}  and ${getsettype} and a.status_code = 1 and d.design_id = $1 group by d.design_id,b.size_id,
                ...
                inner join tbl_item_management as d on d.trans_no=c.trans_no left join tbl_color as f on f.color_id =c.color_id inner join tbl_customer as e ON e.customer_code = a.customer_code where status_flag = 1 and ${designid_val}  and ${getsettype} and a.status_code = 1 and d.design_id = $1 group by d.design_id,b.size_id ,d.item_code,c.qr_code) as t1 where size_id not in (SELECT size_id from (SELECT size_id, coalesce(sum(coalesce(no_of_set,0))- coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where status_flag = 1 and size_id = a.size_id ),0),0) as current_set,coalesce(coalesce((SELECT sum(qty) from tbl_order_taking_items where status_code = 1 and size_id = a.size_id ),0) - coalesce((SELECT sum(dispatch_set) from tbl_dispatch_details where status_flag = 1 and size_id = a.size_id ),0),0) as pending_set from tbl_fg_items as a group by size_id) as dev where current_set > pending_set) group by item_code,design_id,size_id,qr_code )as dev group by item_code,design_id,size_id,qr_code) as d1 where coalesce((qty-fg_set),0) > 0 order by design_id`,[Required_stock_array[i].design_id] );
                let item_Array = item_Result && item_Result.rows ? item_Result.rows : []; 
