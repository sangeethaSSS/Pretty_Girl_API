...
try {
    if (req.jwtToken) { 
    const decoded = await commonService.jwtVerify(req.jwtToken);  
    const {box_array, fg_job_array} = req; 
    if (decoded) { 
      const {user_id} = decoded.data
      const id_max = await client.query(`select coalesce (max(fg_id),0) + 1 as fg_id FROM tbl_fg_items`)
      var fg_id = id_max && id_max.rows[0].fg_id;
