       ...
       if (decoded) { 
        ...
        if(process == 'itemwise'){
          ...
          if (dispatch_array.length > 0) {
            for (let i = 0; i < dispatch_array.length; i++) {
              const item_Result = await client.query(` Select * from (select a.order_no,a.ref_no,to_char(a.order_date,'DD-MM-YYYY') as order_date,a.customer_code, b.size_id,sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details   where status_flag = 1 and order_no=a.order_no),0) as total_piece,d.customer_name,d.mobile_no, coalesce(d.city,'')||' - '||coalesce(d.pincode,'') as city,'0' as dispatch_qty, coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as fg_qty,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as org_fg_qty from tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no  inner join tbl_item_sizes as c on b.size_id=c.size_id inner join tbl_customer as d  on d.customer_code=a.customer_code where  b.size_id= $1 and a.status_code = 1  group by a.order_no,a.ref_no,a.order_date,a.customer_code,b.size_id,d.customer_name,d.mobile_no,d.city,d.pincode) as dev where total_piece > 0 and fg_qty > 0`,[dispatch_array[i].size_id] );
        ...
        if(process == 'customerwise'){
          ...
          item_exec_Result = await client.query(`SELECT customer_code,customer_name  from (
            ...
            d.customer_name,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as fg_qty, sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details where status_flag = 1 and order_no=a.order_no),0) as total_piece  from tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as  c on b.size_id=c.size_id inner join tbl_customer as d on d.customer_code=a.customer_code where a.status_code = 1 and ${customercode_val} and c.size_id in (select size_id from tbl_fg_items) group by a.order_no,a.order_date,a.customer_code,d.customer_name,b.size_id) as dev where total_piece > 0 and fg_qty > 0 group by customer_code,customer_name
          ...
          if (dispatch_array.length > 0) {
            for (let i = 0; i < dispatch_array.length; i++) {
              ...
              tbl_dispatch_details  where status_flag = 1 and order_no=a.order_no),0) as total_piece,'0' as 
    ...
    try {
      if (req.jwtToken) {
        ...
        if (decoded) {   
          ...
          UNION ALL SELECT * FROM (select distinct label,value,sum(total_piece) as total_piece from (select  coalesce(d.customer_name,'') ||' - '||  coalesce(d.mobile_no,'') as label,d.customer_code  as value,sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and order_no=a.order_no),0) as order_qty,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as total_piece,(select  string_agg(fg_id::text,',')  from tbl_fg_items where size_id=b.size_id) as fg_id  from tbl_order_taking as a inner join  tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as c on b.size_id=c.size_id inner join tbl_customer  as d on d.customer_code=a.customer_code and c.size_id in  (select size_id from tbl_fg_items) group by a.order_no,d.customer_name,d.mobile_no,d.customer_code,b.size_id ) as dev where order_qty > 0 and total_piece > 0  group by label,value) as d`);
