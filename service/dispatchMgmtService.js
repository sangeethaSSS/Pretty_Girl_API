    ...
    if (req.jwtToken) {
      ...
      if (decoded) {   
        ...
        if(process == 'excel') { 
          ...
          const item_exec_Result = await client.query(`SELECT dispatch_id,dispatch_no,to_char(dispatch_date, 'dd-MM-YYYY') as dispatch_date,sum(coalesce(order_set,0)) as order_set,sum(order_set*coalesce(b.total_set,'0')::Integer)  as order_pieces,sum(coalesce(dispatch_set,0)) as dispatch_set ,sum(coalesce(dispatch_pieces,0)) as dispatch_pieces,a.user_id,string_agg(distinct customer_name, ',') as customer_name from tbl_dispatch_details a inner join tbl_item_sizes as b ON b.size_id = a.size_id inner join tbl_item_management as c on c.trans_no=b.trans_no inner join tbl_customer as d on                    a.customer_code = d.customer_code inner join tbl_agent as e on d.agent_code = e.agent_code  where 
