     ...
     if (req.jwtToken) {
       ...
       if (decoded) { 
        ...
        if(process == 'itemwise'){
          ...
          item_exec_Result = await client.query(` SELECT size_id,qr_code FROM (SELECT a.order_no,b.size_id,qr_code,to_char(a.order_date,'DD-MM-YYYY') as order_date,sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details   where status_flag = 1 and order_no=a.order_no and size_id = b.size_id),0) as total_piece,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as fg_qty FROM tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as c on b.size_id=c.size_id where a.status_code = 1 and ${sizeid_val} group by a.order_no,b.size_id,qr_code,a.order_date ) as dev where total_piece > 0 and fg_qty > 0 group by size_id,qr_code`)
          ...
          if (dispatch_array.length > 0) {
            for (let i = 0; i < dispatch_array.length; i++) {
              const item_Result = await client.query(` Select * from (select a.order_no,a.ref_no,to_char(a.order_date,'DD-MM-YYYY') as order_date,a.customer_code, b.size_id,sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details   where status_flag = 1 and order_no=a.order_no and size_id = b.size_id),0) as total_piece,d.customer_name,d.mobile_no, coalesce(d.city,'')||' - '||coalesce(d.pincode,'') as city,'0' as dispatch_qty, coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as fg_qty,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as org_fg_qty from tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no  inner join tbl_item_sizes as c on b.size_id=c.size_id inner join tbl_customer as d  on d.customer_code=a.customer_code where a.status_code = 1 and b.size_id= $1 group by a.order_no,a.ref_no,a.order_date,a.customer_code,b.size_id,d.customer_name,d.mobile_no,d.city,d.pincode) as dev where total_piece > 0 and fg_qty > 0`,[dispatch_array[i].size_id] );
        ...
        if(process == 'customerwise'){
          ...
          item_exec_Result = await client.query(`SELECT customer_code,customer_name  from (
            ...
            d.customer_name,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as fg_qty, sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details where status_flag = 1 and order_no=a.order_no and size_id = b.size_id),0) as total_piece  from tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as  c on b.size_id=c.size_id inner join tbl_customer as d on d.customer_code=a.customer_code where a.status_code = 1 and ${customercode_val} and c.size_id in (select size_id from tbl_fg_items) group by a.order_no,a.order_date,a.customer_code,d.customer_name,b.size_id) as dev where total_piece > 0 and fg_qty > 0 group by customer_code,customer_name
          ...
          if (dispatch_array.length > 0) {
            for (let i = 0; i < dispatch_array.length; i++) {
              ...
              dispatch_qty,b.size_id, coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items 
              where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details where status_flag = 1 and size_id=b.size_id),0) as fg_qty,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items 
              where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details where status_flag = 1 and size_id=b.size_id),0) as org_fg_qty from tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as  c on b.size_id=c.size_id inner join tbl_customer as d on d.customer_code=a.customer_code where a.status_code = 1 and lower(a.customer_code) = lower($1) and c.size_id in (select size_id from tbl_fg_items) group by a.order_no,a.ref_no,a.order_date,a.customer_code,b.size_id,c.qr_code,b.size_id) as dev where  total_piece > 0 and fg_qty > 0`,[dispatch_array[i].customer_code] );
              let item_Array = item_Result && item_Result.rows ? item_Result.rows : []; 
              let obj = dispatch_array[i]
              obj['ItemArray'] = item_Array
              result.push(obj)
    ...
    try {
      if (req.jwtToken) {
        ...
        if (decoded) {   
          ...
          const item_exec_Result = await client.query(`SELECT 'All' as label,'0' as value, 0 as total_piece
           UNION ALL SELECT * FROM (select distinct label,value,sum(total_piece) as total_piece from (select qr_code as label,b.size_id as value,
            sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details
            where status_flag = 1 and order_no=a.order_no and size_id = b.size_id),0) as order_qty,
            coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0)
            -coalesce((select  sum(coalesce(dispatch_set,0)) from  tbl_dispatch_details where status_flag = 1 and size_id=b.size_id),0)  as total_piece from tbl_order_taking as a inner join tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as c on b.size_id=c.size_id where a.status_code = 1 group by a.order_no,b.size_id,qr_code) as dev where order_qty > 0  and total_piece > 0 group by label,value order by label) as d`); 
          
          const customer_exec_Result = await client.query(` SELECT 'All' as label,'0' as value, 0 as total_piece
          UNION ALL SELECT * FROM (select distinct label,value,sum(total_piece) as total_piece from (select  coalesce(d.customer_name,'') ||' - '||  coalesce(d.mobile_no,'') as label,d.customer_code  as value,sum(b.qty)::Integer - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and order_no=a.order_no and size_id = b.size_id),0) as order_qty,coalesce((select sum(coalesce(no_of_set,0)) from tbl_fg_items where size_id=b.size_id),0) - coalesce((select sum(coalesce(dispatch_set,0)) from tbl_dispatch_details  where status_flag = 1 and size_id=b.size_id),0) as total_piece,(select  string_agg(fg_id::text,',')  from tbl_fg_items where size_id=b.size_id) as fg_id  from tbl_order_taking as a inner join  tbl_order_taking_items as b on a.order_no=b.order_no inner join tbl_item_sizes as c on b.size_id=c.size_id inner join tbl_customer  as d on d.customer_code=a.customer_code and c.size_id in  (select size_id from tbl_fg_items) where a.status_code = 1 group by a.order_no,d.customer_name,d.mobile_no,d.customer_code,b.size_id ) as dev where order_qty > 0 and total_piece > 0  group by label,value) as d`);
    ...
    if (req.jwtToken) {
      ...
      if (decoded) {   
        ...
        if(process == 'excel') { 
          ...
          const item_exec_Result = await client.query(`SELECT dispatch_id,dispatch_no,to_char(dispatch_date, 'YYYY-MM-dd') as dispatch_date,sum(coalesce(order_set,0)) as order_set,sum(order_set*coalesce(b.total_set,'0')::Integer)  as order_pieces,sum(coalesce(dispatch_set,0)) as dispatch_set ,sum(coalesce(dispatch_pieces,0)) as dispatch_pieces,a.user_id,string_agg(distinct customer_name, ',') as customer_name from tbl_dispatch_details a inner join tbl_item_sizes as b ON b.size_id = a.size_id inner join tbl_item_management as c on c.trans_no=b.trans_no inner join tbl_customer as d on                    a.customer_code = d.customer_code inner join tbl_agent as e on d.agent_code = e.agent_code  where 
          a.dispatch_type = '${filter_type}' and a.status_flag = 1 and  ${dispatch_date} and ${design_code} and ${customer_code} and ${itemcategarycode} and ${agentcode} group by dispatch_id,a.user_id,dispatch_no,dispatch_date order by dispatch_id desc` ); 
          let dispatch_array = item_exec_Result && item_exec_Result.rows ? item_exec_Result.rows : []; 
          const company_Result = await client.query(`SELECT * from tbl_print_setting`);
          let Company_Array = company_Result && company_Result.rows ? company_Result.rows : []; 
          responseData = { "dispatch_array": dispatch_array, "Company_Array":Company_Array, "overallTotal_sets" : overallTotal_sets, "overallTotal_Pieces" : overallTotal_Pieces, "dispatchWidget": dispatch_Widget }
            ...
            const item_total_Result = await client.query(`SELECT dispatch_id,sum(coalesce(order_set,0)) as order_set,sum(coalesce(dispatch_set,0)) as dispatch_set ,a.user_id,dispatch_no,to_char(dispatch_date, 'dd-MM-YYYY') as dispatch_date  from tbl_dispatch_details a inner join tbl_item_sizes as b ON b.size_id = a.size_id inner join tbl_item_management as c on c.trans_no=b.trans_no inner join tbl_customer as d on a.customer_code = d.customer_code inner join tbl_agent as e on d.agent_code = e.agent_code where a.dispatch_type = '${filter_type}' and a.status_flag = 1 and  ${dispatch_date} and ${design_code} and ${customer_code} and ${itemcategarycode} and ${agentcode} group by dispatch_id,a.user_id,dispatch_no,dispatch_date order by dispatch_id desc` );
