...
module.exports.getCustomers = async (req) => {
    ...
    try {
        if (req.jwtToken) {
            ...
            transport_contact_no, transport_location, user_id,transport_contact_person,
            (select user_name from tbl_user where user_id = (select user_id from "tbl_userlog" 
          where autonum = maker_id limit 1) limit 1) as employeename,		  
		  (select coalesce(to_char(log_date,'DD-MM-YYYY HH12:MI PM'),'') from "tbl_userlog" 
